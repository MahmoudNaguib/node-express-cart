...
const factory = require("../../../Database/Factories/Favorite");
...
let createRecord = async () => {
    let user = await getUser();
    let product = await knex('products').orderBy('id', 'desc').first();
    let created = await knex('favorites').insert(await factory.generate({user_id: user.id, product_id: product.id}));
    if (created) {
        return await knex('favorites').where('id', created[0]).first();
    }
}
let deleteRecord = async (record) => {
    return await knex('favorites').where('id', record.id).del();
}
...
describe("Favorites", () => {
    ...
    test("store", async () => {
        ...
        expect(res.body).toHaveProperty('data');
        let record = await knex('favorites').where('id', res.body.data.id).first();
        deleteRecord(record);
    });
    ...
    test("delete", async () => {
        ...
        let user = await getUser();
        const res = await request(app)
            .delete("/api/favorites/" + record.id)
            .set('Authorization', 'Bearer ' + user.token);
        expect(res.statusCode).toBe(200);
        expect(res.body).toHaveProperty('message');
    });
